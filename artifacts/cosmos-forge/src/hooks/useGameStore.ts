...
interface BlackHoleAlert { timeLeft: number; }
...
const mkInitial = () => ({
  ...
  blackHoleAlert: null as BlackHoleAlert | null,
  deathReason: '',
...
function getDeathReason(eventId: string | undefined, era: Era): string {
  const reasons: Record<string, string> = {
    ...
    resource_collapse: 'you consumed faster than the planet could heal. the math caught up.',
    signal_war: 'the aliens you contacted sent warships instead of handshakes.',
  };
  if (reasons[eventId ?? '']) return reasons[eventId ?? ''];
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  advanceTime: (years) => set((state) => {
    ...
    const updates: Partial<GameState> & { deathReason?: string } = { year: newYear };

    // Unlock one suggestion at a time
    ...
    // Overpopulation / resource collapse
    if ((updates.population ?? state.population) > 800_000_000_000) {
      const collapse = Math.random() < 0.25;
      if (collapse) {
        updates.phase = 'lose';
        updates.deathReason = 'resource_collapse';
        updates.deathReason = 'population exceeded all sustainable limits. the biosphere collapsed in a single decade.';
      }
    }

    // Tech overdose death (becoming too powerful)
    if (state.tech >= 95 && state.energy >= 88 && !state.activeEvent && Math.random() < 0.10) {
      const singularityEvent = RANDOM_EVENTS.find(e => e.id === 'tech_singularity');
      if (singularityEvent) updates.activeEvent = singularityEvent;
    }

    // Random event selection — filter by population/era
    const hasHumans = (updates.population ?? state.population) >= 200_000_000;
    ...
    if (Math.random() < eventChance && !state.activeEvent && !(updates.activeEvent) && state.population > 0) {
      const pool = RANDOM_EVENTS.filter(e => {
        if (e.id === 'tech_singularity') return false; // handled above
    ...
    // Black hole trigger (advanced civilizations only)
    const bhChance = years >= 1_000_000_000 ? 0.05 : 0.02;
    if (!state.blackHoleAlert && state.tech >= 35 && state.year >= 4_000_000_000 && Math.random() < bhChance) {
      updates.blackHoleAlert = { timeLeft: 40 };
      updates.logs = [
        { id: Math.random().toString(36), time: `yr ${fmtYear(newYear)}`, text: '⚫ WARNING: gravitational anomaly detected — black hole approaching!' },
        ...state.logs
      ].slice(0, 80);
    }

    // Health/population death checks
  ...
  completeSuggestion: (id) => set((state) => {
    ...
    // Federation no longer wins — gives transcendence bonus and continues
    delete effects.win;
    return {
  ...
  escapeBlackHole: () => set((state) => ({
    blackHoleAlert: null,
    logs: [
      { id: Math.random().toString(36), time: `yr ${fmtYear(state.year)}`, text: '⚡ emergency thrust! your planet escaped the black hole\'s pull.' },
      ...state.logs
    ].slice(0, 80),
  })),
  ...
  tickBlackHole: () => set((state) => {
    ...
    if (newTime <= 0) {
      return {
        blackHoleAlert: null,
        phase: 'lose',
        deathReason: 'your planet crossed the event horizon. the gravitational tide tore it apart. nothing escaped.',
      };
    }
  ...
  dismissSignalResponse: () => set((state) => {
    ...
    // Signal responses can also cause death (alien war)
    if (effAny.population === 0 || effAny.health <= 0) {
      return {
        ...effects,
        pendingSignalResponse: null,
        phase: 'lose',
        deathReason: resp.type === 'war'
          ? 'your signal was an act of war. the fleet arrived before you could prepare. not a building was left standing.'
          : resp.type === 'invasion'
          ? 'they followed the signal home. what began as curiosity ended in conquest. your civilization was absorbed.'
